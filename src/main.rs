...
fn main() {
    let matches = clap::App::new("Simple HTTP(s) Server")
        ...
        .arg(clap::Arg::with_name("drive_dir")
            .short("d")
            .long("drive_dir")
             .index(1)
             .validator(|s| {
    ...
    let root = Path::new("www").to_path_buf();
    let index = true;//matches.is_present("index");
    ...
    ======== [{}] ========"#,
                &vec![
                    ...
                    drive_dir.to_str().unwrap().to_owned(),
...
impl Handler for MainHandler {
    fn handle(&self, req: &mut Request) -> IronResult<Response> {
        let pathes = req.url.path();
        let read_from_drive = pathes[0] == "_drive";
        let mut fs_path = if read_from_drive {
            self.drive_dir.clone()
        }else{
            self.root.clone()
        };
        ...
        /*if !fs_path.starts_with(&self.root) || !fs_path.starts_with(&self.drive_dir) {
        ...
        let params: HashMap<String, String> = url::form_urlencoded::parse(req.url.query().unwrap_or("").as_bytes())
            .into_owned()
            .collect();
        //直接返回文件尺寸
        if params.contains_key("size"){
            return Ok(Response::with((status::Ok, get_file_size(&fs_path.to_path_buf()).unwrap_or(0).to_string())))
        }
        //直接返回文件时间
        if params.contains_key("mod_time"){
            return Ok(Response::with((status::Ok, get_file_mod_time(&fs_path.to_path_buf()).unwrap_or(0).to_string())))
        }
        if params.contains_key("thumbnail"){
...
fn get_file_mod_time(file_path: &PathBuf) -> Result<u64,Box<dyn Error>> {
...
fn get_file_size(file_path: &PathBuf) ->  Result<u64,Box<dyn Error>>{
    let metadata = fs::metadata(file_path)?;
    let size = if file_path.is_dir(){
        fs_extra::dir::get_size(file_path).unwrap_or(0)
    } else {
        metadata.len()
    };
    Ok(size)
